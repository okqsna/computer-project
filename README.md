...
Якщо він є неорієнтованим(ребра представлені множинами), то створюється новий список і у нього додається два ребра: в одну та іншу сторону.
Якщо він орієнтований(ребра представлені кортежами), то іде тоді перевірка, чи цей орієнтований граф є зв'язним. Якщо він не є зв'язним, то функція скаже про це і припинить своє виконання.

#### Рекурсивна функція ```calculate_way```.

В основі цієї функції лежить алгоритм Флері.

Вона починає рухатись від початкової вершини, і перебирає варіанти куди можна рухатись далі з точки, в якій ми знаходимось. Коли вона пройшлась раз по ребру, то це ребро і його дзеркальна копія, видаляються з можливих подальших напрямків для цього шляху. Так вона генерує всі можливі шляхи. 
Коли ж він попадає у вершину, звідки й виходив, та довжина цього шляху є такою, як довжина початкового графу(тобто він містить всі ребра), то він записується до списку, який містить всі можливі цикли ейлера.
